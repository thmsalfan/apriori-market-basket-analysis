...
if (isset($_POST["submit"])) {
    $id = $_SESSION['apriori_id_reset'];
    $password = $_POST['password'];
    $confirm_password = $_POST['confirm_password'];

    if ($password == $confirm_password) {
        $encrypted = md5($confirm_password);

        $sql = "UPDATE `users` SET `password`='$encrypted' WHERE id = $id";
        $result = mysqli_query($conn, $sql);

        if ($result) {
            header('Location: clear-session.php?s=2');
        } else {
            $komen = "error";
        }
    } else {
                    ...
                    <div class="position-relative">
                        <div id="login-box" class="login-box visible widget-box no-border">
                            <div class="widget-body">
                                ...
                                if (isset($komen)) {
                                    if ($komen == 'tidak sama') {
                                        display_error("Password Tidak Sama");
                                    } else {
                                        display_error("Terjadi Kesalahan");
                                    }
                                }
                                ?>
                                    ...
                                    <div class="col-sm-6">
                                        ...
                                        <form method="post" action="" >
                                            <fieldset>
                                                ...
                                                <div class="clearfix">
                                                    <a href="clear-session.php?s=1">Batal</a>
